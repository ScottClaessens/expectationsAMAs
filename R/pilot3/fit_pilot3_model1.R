...
fit_pilot3_model1 <- function(pilot3_data) {
  ...
  brm(
    ...
    prior = c(
      ...
      prior(exponential(3), class = sd, resp = trustbaseline),
      prior(exponential(3), class = sd, resp = trustotherissuesbaseline),
      prior(exponential(3), class = sd, resp = empathybaseline),
      prior(exponential(3), class = sd, resp = competencybaseline),
      prior(exponential(3), class = sd, resp = trustoverall),
      prior(exponential(3), class = sd, resp = trustotherissuesoverall),
      prior(exponential(3), class = sd, resp = empathyoverall),
      prior(exponential(3), class = sd, resp = competencyoverall),
      prior(exponential(3), class = sd, resp = likelyhuman),
      prior(exponential(3), class = sd, resp = surprisedAI),

...
# comprehension check codes
comp_codes <- c("All advisors were human", "All advisors were AI",
...
d <-
  read_survey(
    file_name = 
      "Expectations+AMAs+-+Study+3_December+16,+2025_04.47.csv",
    add_var_labels = FALSE
  ) |>
  # remove repeated participant (keep first submission)
  filter(ResponseId != "R_8rJIBq1gzYTidbW") |>
  # add id column
  rowid_to_column("id")
...
d |>
  ...
  transmute(
    ...
    advisor_choice           = ifelse(Choice == 1, AdvisorA, NA),
    advisor_choice           = ifelse(Choice == 2, AdvisorB, advisor_choice),
    advisor_choice           = ifelse(Choice == 3, AdvisorC, advisor_choice),
    advisor_choice           = ifelse(Choice == 4, AdvisorD, advisor_choice),
    ...
    check_condition          = comp_codes[ComprehensionCond],
  ...
  write_csv(file = "study3_data_clean.csv")
